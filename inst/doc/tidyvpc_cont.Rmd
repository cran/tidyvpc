...
LOESS prediction corrected using binless method for 10%, 50%, 90% quantiles. If `optimize = TRUE`, the LOESS smoothing parameter, span, will be automatically optimized using AIC. Note: `predcorrect()` must be called before `binless()` if setting `loess.ypc = TRUE`.
```{r warning=FALSE, fig.width = 9, fig.height = 6, out.width=640}
vpc <- observed(obs_data, x=TIME, y=DV) %>%
    simulated(sim_data, y=DV) %>%
    stratify(~GENDER) %>%
...
To specify your own span value LOESS pcVPC instead of optimizing with AIC, use the `span` argument in the `binless` function. Span should be a numeric value between [0,1], with higher values providing a smoother fit. Remember, to also include the smoothing parameters for AQR by using the `lambda` argument and set `optimize = FALSE`.
